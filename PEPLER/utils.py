...
class DataLoaderKGAT:
    def __init__(self, data_path, index_dir, tokenizer, seq_len):
        self.user_org_id_remap_id = {}
        self.item_org_id_remap_id = {}
    ...
    def initialize(self, data_path):
        ...
        with open('/home/es20btech11035/VikAmanXML/knowledge_graph_attention_network/Data/yelp2018/user_list.txt', 'r') as file:
            next(file)
            for line in file:
                org_id, remap_id = line.strip().split()
                self.user_org_id_remap_id[org_id] = int(remap_id)
        with open('/home/es20btech11035/VikAmanXML/knowledge_graph_attention_network/Data/yelp2018/item_list.txt', 'r') as file:
            next(file)
            for line in file:
                org_id, remap_id = line.strip().split()
                self.item_org_id_remap_id[org_id] = int(remap_id[:-len(org_id)])
        embeds = np.load('/home/es20btech11035/VikAmanXML/knowledge_graph_attention_network/Model/pretrain/yelp2018/kgat-fix.npz')
        self.user_embed = embeds['user_embed']
        self.item_embed = embeds['entity_embed']
    ...
    def load_data(self, data_path, index_dir):
        ...
        for review in reviews:
            ...
            data.append({'user': self.user_embed[self.user_org_id_remap_id[review['user']]],
                        'item': self.item_embed[self.item_org_id_remap_id[review['item']]],
                        'rating': review['rating'],
                        'text': text,
                        'feature': fea})
...
class Batchify:
    def __init__(self, data, tokenizer, bos, eos, batch_size=128, shuffle=False):
        ...
        self.user = torch.tensor(u, dtype=torch.float32).contiguous()
        self.item = torch.tensor(i, dtype=torch.float32).contiguous()
