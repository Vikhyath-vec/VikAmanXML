...
def test(sess, model, users_to_test, drop_flag=False, batch_test_flag=False):
    ...
    for u_batch_id in range(n_user_batchs):
        ...
        if batch_test_flag:
            ...
            feed_dict = data_generator.generate_test_feed_dict(model=model,   #puts the input data into a dict 
                                                               user_batch=user_batch,
                                                               item_batch=item_batch,
                                                               drop_flag=drop_flag)
...
# def aman_test(sess, model, users_to_test, drop_flag=False, batch_test_flag=False):
#     result = {'precision': np.zeros(len(Ks)), 'recall': np.zeros(len(Ks)), 'ndcg': np.zeros(len(Ks)),
#               'hit_ratio': np.zeros(len(Ks)), 'auc': 0.}
...
#     test_users = users_to_test
#     n_test_users = len(test_users)
#     n_user_batchs = 1 #n_test_users // u_batch_size + 1
...
#     return item_batch, user_batch, rate_batch