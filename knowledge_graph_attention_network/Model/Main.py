    ...
    *********************************************************
    ...
    for epoch in range(args.epoch):
        ...
        *********************************************************
        ...
        """
        for idx in tqdm(range(n_batch), colour='green'):
            btime= time()
        ...
        *********************************************************
        ...
        if args.model_type in ['kgat']:
            ...
            if args.use_kge is True:
                ...
                for idx in tqdm(range(n_A_batch), colour='red'):
                    btime = time()
        ...
        *********************************************************
        ...
        if args.save_flag == 1:
            ...
            user_embed1, entity_embed1, relation_embed1 = sess.run(
                        [model.weights['user_embed'], model.weights['entity_embed'], model.weights['relation_embed']],
                        feed_dict={})
            temp_save_path1 = '%spretrain/%s/%s.npz' % (args.proj_path, args.dataset, args.model_type)
            ensureDir(temp_save_path1)
            np.savez(temp_save_path1, user_embed=user_embed1, entity_embed=entity_embed1, relation_embed=relation_embed1)
            print('save the weights of kgat in path: ', temp_save_path1)
